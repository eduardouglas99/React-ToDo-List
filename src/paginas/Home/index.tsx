...
const Home = () => {
  ...
  function handleTarefaConcluida(
    id: number,
    titulo: string,
    concluido: boolean
  ) {
  ...
  function clearTask(item: Tarefa) {
    ...
    const novoArray = listaTarefas.filter(
  ...
  function undoAction(itemSalvo: Tarefa) {
    const temItemIgualNoArray = listaTarefas.filter(
      (item) => item.titulo === itemSalvo.titulo
    ).length;
          ...
          <TarefaProvider>
            ...
            {feedback && (
              <p className={`${styles.container__modalCenter__feedbackError}`}>
                ...
                {listaTarefas.length > 0 ? `nova tarefa.` : `tarefa.`}
              </p>
            )}
            <ListTask
              clearTask={clearTask}
